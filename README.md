...
| `SPEC-bigmoe.md` | Design spec for RAM-offload MoE models (DeepSeek-V4-Flash) |
...
| Model ID | GPUs | Quant | On disk | Context | Notes |
|---|---|---|---|---|---|
| `deepseek-v4-flash` | both 3090s + RAM | `UD-Q4_K_XL`, 5 shards | 144.4 GiB | 65536, 1 slot | default |
| `deepseek-v4-flash-q3` | both 3090s + RAM | `UD-Q3_K_M`, 4 shards | 119.3 GiB | 65536, 1 slot | capacity fallback, lossy |

Both use `unsloth/DeepSeek-V4-Flash-0731-GGUF` (the 0731 checkpoint, which ships the DSpark
drafter), the DSpark drafter on the GPUs, and `ttl: 8 h`. Like the other whole-box entries
they are in no matrix set, so they run alone and **any** card request evicts them. Only the
on-call poller is told to leave them alone.
